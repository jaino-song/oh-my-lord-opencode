 ...
 * @returns {string} Package name like "oh-my-lord-opencode-darwin-arm64"
...
export function getPlatformPackage({ platform, arch, libcFamily }) {
  ...
  return `oh-my-lord-opencode-${os}-${arch}${suffix}`;
}
 ...
 * @returns {string} Relative path like "oh-my-lord-opencode-darwin-arm64/bin/oh-my-lord-opencode"
 */
export function getBinaryPath(pkg, platform) {
  const ext = platform === "win32" ? ".exe" : "";
  return `${pkg}/bin/oh-my-lord-opencode${ext}`;
}
