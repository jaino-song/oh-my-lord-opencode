...
export function formatVersionOutput(info: VersionInfo): string {
  ...
  lines.push("")
   lines.push(color.bold(color.white("oh-my-lord-opencode Version Information")))
  ...
  switch (info.status) {
    ...
    case "outdated":
      lines.push(`  ${SYMBOLS.warn} ${color.yellow("Update available")}`)
       lines.push(`  ${color.dim("Run:")} ${color.cyan("cd ~/.config/opencode && bun update oh-my-lord-opencode")}`)
      break
    case "local-dev":
