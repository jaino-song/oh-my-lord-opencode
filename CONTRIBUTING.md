...
First off, thanks for taking the time to contribute! This document provides guidelines and instructions for contributing to oh-my-lord-opencode.
...
2. **Update your OpenCode config** (`~/.config/opencode/opencode.json` or `opencode.jsonc`):
   ...
   For example, if your project is at `/Users/yourname/projects/oh-my-lord-opencode`:
   ```json
   {
     "plugin": [
       "file:///Users/yourname/projects/oh-my-lord-opencode/dist/index.js"
     ]
   }
   ```

   > **Note**: Remove `"oh-my-lord-opencode"` from the plugin array if it exists, to avoid conflicts with the npm version.
